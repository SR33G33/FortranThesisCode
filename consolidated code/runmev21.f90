...
IF (QC(N,J)) 29,19,29
19 CONTINUE
WRITE (6, 801) "THE SYSTEM IS NOT COMPLETELY CONTROLLABLE"
801 FORMAT (//,5X)
GO TO 1500
29 IC=(J-1)/IR
WRITE (6, 70) "MATRIX QC", ((QC (I,J), J=1,NR), I=1,N)
70 FORMAT (20X, 15 (1X,F5.2))
...
IF (QO(N,J)) 49,39,49
39 CONTINUE
WRITE (6, 901) "SYSTEM IS NOT COMPLETELY OBSERVABLE"
901 FORMAT (//, 5X)
GO TO 1500
49 IO=(J-1)/M
WRITE (6, 90) "MATRIX QO" ,((QO(I,J), J=1, NM), I=1,N)
90 FORMAT (20X, /10(3X, E 9.2))
DO 310 I=1,IR
DO 310 J=1,M
310 AK1 (I,J)=0
CALL ZADEH (A, N, AL)
...
311 WRITE (6, 312) "MATRIX A IS CYCLICAL"
312 FORMAT (//, 10X)
DO 313 I=1,N
DO 313 J=1,N
313 A1(I,J)=A(I,J)
...
325 WRITE (6, 326) "MATRIX Y IS NOT CYCLICAL"
326 FORMAT (//, 10X)
K=2
331 NM=2*N-K
CALL PRIM (RR, Q, N, NM, K)
...
334 S=S+1.
GO TO 336
324 WRITE (6, 340) "Y HAS BECOME CYCLICAL"
340 FORMAT (//,10X)
DO 341 I=1,N
DO 341 J=1, N
341 A1 (I,J)=Y(I,J)
...
S=S+1.
GO TO 405
404 WRITE (6, 800) "G COULD NOT BE FOUND"
800 FORMAT (//, 5X)
GO TO 1500
401 K=1
S=S+1.3
...
S=S+1.
GO TO 410
409 WRITE (6,900)  "T COULD NOT BE FOUND"
900 FORMAT (//, 5X)
GO TO 1500
...
180 FORMAT (20X,I2,F8.2, 10X,I2,F8.2)
1500 STOP
END
